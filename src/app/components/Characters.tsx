// ─────────────────────────────────────────────────────────────────────────────
// Characters.tsx
// SVG character illustrations with dynamic emotions for the QC Heat Risk Monitor.
// Each character accepts a `mood` prop: "happy" | "neutral" | "stressed"
// ─────────────────────────────────────────────────────────────────────────────

export type Mood = "happy" | "neutral" | "stressed";

// Shared face helper — generates eyes, mouth, brows based on mood
function Face({ cx, cy, mood, skinDark = "#333", eyeFill = "#222" }: {
  cx: number; cy: number; mood: Mood; skinDark?: string; eyeFill?: string;
}) {
  const lx = cx - 8, rx = cx + 8, my = cy + 10;
  const browY = cy - 5;
  return (
    <>
      {/* Eyes */}
      <ellipse cx={lx} cy={cy} rx={2.8} ry={mood === "stressed" ? 2.2 : 3} fill="white" />
      <ellipse cx={rx} cy={cy} rx={2.8} ry={mood === "stressed" ? 2.2 : 3} fill="white" />
      <circle cx={lx + 0.5} cy={cy + 0.5} r={1.5} fill={eyeFill} />
      <circle cx={rx + 0.5} cy={cy + 0.5} r={1.5} fill={eyeFill} />
      <circle cx={lx + 1} cy={cy} r={0.6} fill="white" />
      <circle cx={rx + 1} cy={cy} r={0.6} fill="white" />
      {/* Brows */}
      {mood === "stressed" && (
        <>
          <line x1={lx - 3} y1={browY + 2} x2={lx + 3} y2={browY} stroke={skinDark} strokeWidth="1.5" strokeLinecap="round" />
          <line x1={rx - 3} y1={browY} x2={rx + 3} y2={browY + 2} stroke={skinDark} strokeWidth="1.5" strokeLinecap="round" />
        </>
      )}
      {mood === "neutral" && (
        <>
          <line x1={lx - 3} y1={browY} x2={lx + 3} y2={browY} stroke={skinDark} strokeWidth="1.2" strokeLinecap="round" />
          <line x1={rx - 3} y1={browY} x2={rx + 3} y2={browY} stroke={skinDark} strokeWidth="1.2" strokeLinecap="round" />
        </>
      )}
      {/* Mouth */}
      {mood === "happy" && (
        <path d={`M${lx} ${my} Q${cx} ${my + 6} ${rx} ${my}`} fill="none" stroke={skinDark} strokeWidth="1.8" strokeLinecap="round" />
      )}
      {mood === "neutral" && (
        <line x1={lx + 1} y1={my + 2} x2={rx - 1} y2={my + 2} stroke={skinDark} strokeWidth="1.8" strokeLinecap="round" />
      )}
      {mood === "stressed" && (
        <path d={`M${lx} ${my + 4} Q${cx} ${my - 1} ${rx} ${my + 4}`} fill="none" stroke={skinDark} strokeWidth="1.8" strokeLinecap="round" />
      )}
      {/* Sweat drops when stressed */}
      {mood === "stressed" && (
        <>
          <ellipse cx={cx - 14} cy={cy - 8} rx={2} ry={3} fill="rgba(100,200,255,0.7)" />
          <ellipse cx={cx + 12} cy={cy - 5} rx={1.5} ry={2.5} fill="rgba(100,200,255,0.6)" />
        </>
      )}
    </>
  );
...
export function ConstructionWorker({ mood = "happy" }: { mood?: Mood }) {
  return (
    <svg viewBox="0 0 120 220" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Face */}
      <Face cx={60} cy={61} mood={mood} skinDark="#333" eyeFill="#222" />
      {/* Ears */}
...
export function TrafficEnforcer({ mood = "happy" }: { mood?: Mood }) {
  return (
    <svg viewBox="0 0 120 220" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      {/* Cap */}
      ...
      {/* Face */}
      <Face cx={60} cy={62} mood={mood} skinDark="#6B3A2A" eyeFill="#2A1A0E" />
      {/* Earrings */}
...
export function Student({ mood = "happy" }: { mood?: Mood }) {
  return (
    <svg viewBox="0 0 120 220" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      {/* Hair */}
      ...
      {/* Face */}
      <Face cx={60} cy={62} mood={mood} skinDark="#333" eyeFill="#222" />
      {/* Rosy cheeks */}
      ...
      <g transform="translate(36, 102) scale(0.8)">
        ...
        <path d="M6 6 L8 7 L7 9 L5 8 Z" fill="#1B4D3E" />
...
export function StreetVendor({ mood = "happy" }: { mood?: Mood }) {
  return (
    <svg viewBox="0 0 120 220" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      {/* ── THE VENDOR (Behind) ── */}
      <ellipse cx="80" cy="65" rx="18" ry="20" fill="#C68642" />
      <ellipse cx="80" cy="79" rx="12" ry="7" fill="#A0693A" opacity="0.3" />
      <Face cx={80} cy={62} mood={mood} skinDark="#3D1A00" eyeFill="#1A0E00" />
...
export function Civilian({ mood = "happy" }: { mood?: Mood }) {
  return (
    <svg viewBox="0 0 120 220" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      {/* Short hair */}
      ...
      {/* Face */}
      <Face cx={60} cy={65} mood={mood} skinDark="#555" eyeFill="#333" />
      {/* Neck */}
